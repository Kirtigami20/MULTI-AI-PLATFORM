...
export default function ToolCreate() {
  ...
  const removeParam = (id) => {
    setForm((prev) => ({
      ...
      config: {
        ...prev.config,
        parameters: prev.config.parameters.filter((p) => p.id !== id),
      },
    }))
  }
  ...
  const handleSubmit = (e) => {
    ...
    if (form.tool_type === 'api') {
      ...
      try {
        ...
        createMutation.error = { message: 'Invalid JSON in headers field.' }
        return
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {createMutation.isError && (
          <div className="rounded-lg bg-destructive/10 px-4 py-3 text-sm text-destructive">
            {createMutation.error?.response?.data?.detail || 'Failed to create tool.'}
          </div>
        )}
